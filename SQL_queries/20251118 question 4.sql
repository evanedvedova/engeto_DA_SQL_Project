...
FROM wage_avg_year AS y0
LEFT JOIN wage_avg_year AS y1
...
FROM price_avg_year AS y0
LEFT JOIN price_avg_year AS y1
...
FROM wage_change_per_year AS w
LEFT JOIN price_change_per_year AS p
ON w.record_year = p.record_year
WHERE wage_pct_change IS NOT NULL 
AND price_pct_change IS NOT null
GROUP BY w.record_year, wage_pct_change 
ORDER BY w.record_year ASC
;