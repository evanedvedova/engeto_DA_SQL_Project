...
SELECT 
    ...
    gdp
FROM 
    t_eva_nedvedova_project_sql_secondary_final AS s
),
gdp_change AS (
SELECT
    curr.country,
...
lagged AS (
    ...
    FROM 
        gdp_change g
    LEFT JOIN 
        wage_change_per_year w USING(record_year)
    LEFT JOIN
        price_change_per_year p USING(record_year)
    WHERE 
...
SELECT 
    ...
    ROUND(
        regr_slope(
            ...
            gdp_pct_change::numeric)::numeric,
    2) AS slope,
    ROUND(
        regr_intercept(
            wage_pct_change::numeric,
